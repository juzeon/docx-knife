...
AI Agent 直接复现长文本并用原文字符串定位，会同时放大幻觉、定位失败、格式破坏与上下文占用。该变更建立一个 Python DOCX 补丁引擎：LLM 只生成带段落 ID 的结构化操作，执行器独占 OOXML 解析、文本映射、结构保护、原子回滚与安全保存。

约束包括：只修改 `word/document.xml`；正文与表格单元格内段落进入编辑面；SDT、页眉页脚、脚注尾注、批注、文本框、图片、字段刷新和修订标记生成不在范围内；未命中 XML 保证语义及结构不变，但不承诺字节级序列化一致。
...
- 以实例内段落 ID 解耦定位与内容复现。
- 提供跨 run 的可见文本映射和确定性段内编辑。
- 提供段落级多段插入、删除、替换与 raw XML 可信调用模式。
- 通过预校验、冲突检测、快照和提交前验证实现全批次原子性。
- 通过源漂移检测、备份、ZIP 重验和原子替换保证保存安全。
- 将公共 API、错误、日志、测试和性能契约固定为可实现规范。

**Non-Goals:**

- 编辑 SDT 后代、页眉页脚、脚注尾注、批注、文本框、图片或其他 DOCX part。
- 生成修订标记、刷新字段或代理第三方编辑器能力。
- 向 LLM 暴露 XML、XPath、数组下标、`w14:paraId` 或 raw 模式。
- 保证 OOXML 序列化结果与输入逐字节一致。

## Decisions

以下完整设计是本变更的权威技术决策。它保留原始 `spec/design.md` 的全部行为、边界、API、算法、异常、验收测试和工程质量要求。


## 1. 核心行为

系统以段落 ID 定位，以原子批次提交编辑，并在保存前备份旧文件。LLM 只生成结构化操作，不接触 XML、XPath、数组下标或 `w14:paraId`。未命中 XML 的保真契约是语义及结构不变，不承诺解析、序列化后的字节表示不变。
...
以下结构不进入编辑面：SDT、修订标记生成、页眉页脚、脚注尾注、批注、文本框、图片、字段刷新。SDT 后代段落不分配 ID，也不出现在段落查询结果中；计算全局序号和表格位置时仍保留其结构占位。编辑范围穿越不支持结构时返回 `UnsupportedStructureError`，不降级为破坏性重建。
...
```text
paragraph_count() -> int
list_paragraphs(start=1, limit=None, max_chars=80, raw=False) -> ParagraphListResult
grep_paragraphs(pattern, regex=False, start=1, limit=None,
                max_chars=0, raw=False) -> ParagraphSearchResult
get_paragraph(paragraph_id, raw=False) -> str
get_visible_text(raw=False) -> str
find_text(pattern, regex=False, occurrence=None,
          paragraph_id=None, raw=False) -> TextMatch | list[TextMatch] | None
count_matches(pattern, regex=False, paragraph_id=None, raw=False) -> int
```
...
所有段落读取接口都有 `raw` 参数。`raw=false` 时读取可见文本；`raw=true` 时返回包含起止标签的完整 `<w:p>...</w:p>`，不执行 TextMap 投影，也不生成或转义 `[[DOCX:...]]` 保留标记。返回值中的匹配范围和截断长度基于 XML 字符串。`list_paragraphs()` 与 `grep_paragraphs()` 的结果项分别使用 `text` 或 `xml` 字段，二者不同时出现。`get_visible_text(raw=true)` 按文档顺序连接各段 `<w:p>` XML，不添加 XML 声明或包装节点。原始模式面向可信调用方，不进入 LLM 上下文。
...
所有读写段落内容的接口和 `EditOperation` 都有 `raw` 参数，默认 `false`。`raw=false` 时内容是普通文本；`raw=true` 时输入是由一个或多个完整 `<w:p>...</w:p>` 组成的 XML fragment。一个操作内的输入内容和返回预览必须使用同一模式，禁止混用文本与原始 XML。原始写入在修改 DOM 前解析整个 fragment，并校验每个顶层节点都是 `<w:p>`。`paragraph_count()`、`delete_para()` 等不读取或写入内容的接口不接受无意义的 `raw` 参数。
...
段落操作名包含 `para`，与段内文本操作明确区分。插入和替换统一接收 `items` 数组；单段内容也使用单元素数组。默认模式下，一个 item 可因段落分隔换行展开为多个段落；原始模式下，一个 item 可包含多个连续的 `<w:p>`。执行器按 item 顺序及每个 item 内的段落顺序展平后写入。
...
```json
{
  ...
  "target_id": "p_000042",
  "items": [
    {"content_literal": "新的第一段"},
    {"content_literal": "新的第二段"}
  ]
}
```
...
`Paragraph.replace_para(items, raw=False)` 和 `Paragraph.delete_para()` 是单目标便利接口，分别委托给文档级接口。`insert_para_before()`、`insert_para_after()` 和 `replace_para()` 在文档级与对象级接口中都接受 `raw`；返回的 `Paragraph` 可通过 `read(raw=False)` 读取可见文本，或通过 `read(raw=True)` 读取完整 `<w:p>`。
...
6. 新文本继承插入点或首个被替换字符所在 run 的 `<w:rPr>`；完整的 `[[DOCX:...]]` 保留标记复原为对应 XML 节点；
...
默认模式接收普通文本，由引擎构造合法 OOXML。段落操作直接接收 `list[str]`；JSON 操作中的每个 `items` 元素使用 `content_literal` 或 `content_ref`，必须且只能提供一个。

`raw=true` 时不构造段落或 run，也不识别 `[[DOCX:...]]`。读取结果和写入内容都包含完整 `<w:p>...</w:p>`；每个 item 可以包含一个或多个连续的 `<w:p>`，调用方可自由构造段落内部 XML。引擎只要求 fragment XML 良构、顶层节点全部为 WordprocessingML `<w:p>`，不限制段落内部节点。段内操作只处理可见文本，不接受 `raw=true`；需要修改 XML 时先读取原始段落，再通过 `replace_para(..., raw=true)` 写回。
...
Python API 的读取、插入和替换方法都接受同名 `raw` 参数：

```python
xml = doc.get_paragraph(target.id, raw=True)  # 包含 <w:p> 起止标签
fragment = xml + '<w:p xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:r><w:t>新增段落</w:t></w:r></w:p>'
replacement = doc.replace_para(target.id, [fragment], raw=True)
```

执行器先解析引用，再按操作的 `raw` 模式处理结果。`raw=false` 时先把 `\r\n` 和 `\r` 统一为 `\n`：单个 `\n` 写为当前段落中的 `<w:br/>`，连续两个及以上 `\n` 作为一个段落边界。换行规则同时适用于 `content_literal` 和 `content_ref`，因此一个 item 可以生成多个段落。`raw=true` 时引用结果按 XML fragment 解析，换行仅作为 XML 空白处理，不触发分段或 `<w:br/>` 转换。

缺失键、文件不存在、命令执行失败，以及要求单值却命中多值时拒绝执行。`content_literal` 与 `content_ref` 必须且只能提供一个。原始模式不提供模板注册表或 `template` 类型引用；调用方直接传入完整 XML，或通过受控的 `file`、`command` 引用加载。
...
抽取按文档顺序拼接 `<w:t>`，并把非文本可见节点投影为以下保留标记：

```text
<w:tab/>                 -> [[DOCX:TAB]]
<w:br/>                  -> [[DOCX:LINE_BREAK]]
<w:br w:type="page"/>    -> [[DOCX:PAGE_BREAK]]
<w:br w:type="column"/>  -> [[DOCX:COLUMN_BREAK]]
<w:cr/>                  -> [[DOCX:CR]]
```

正文原本包含的 `[[DOCX:` 投影为 `\[[DOCX:`，写回时反向解码。保留标记是不可拆分的 TextMap 单元：匹配边界落在标记内部时拒绝操作。替换内容中的完整保留标记写回对应 XML 节点，其他文本写入 `<w:t>`；未知或不完整的 `[[DOCX:...]]` 标记返回 `InvalidContentError`。

既有修订内容固定采用 Word 最终视图：包含 `<w:ins>`、排除 `<w:del>`。字段、超链接、书签等结构记录在命中 metadata 中，供能力矩阵决定允许、保留或拒绝。
...
cursor = anchor
for item in items:
    for paragraph in expand_item(item, raw=operation.raw):
...
原始模式按输入 fragment 写入，不复制锚点的 `<w:pPr>` 或 `<w:rPr>`，不执行文本规范化。输入中的每个顶层 `<w:p>` 独立获得新 ID。
...
每条段落插入或替换操作携带有序 `items` 数组。最终顺序由 item 顺序及每个 item 展开后的段落顺序共同确定，不再引入额外排序字段。
...
- 全部操作已消费，目标状态符合预期；
- 段落数量变化与操作一致；
- 未命中节点的 XML 语义及结构不变；比较时对 XML 规范化，不比较命名空间前缀、属性顺序或等价的序列化形式；
- 每项操作的结果和警告均已记录。
...
InvalidContentError(raw, reason)
...
LLM 输入仅包含用户要求、相关段落、可用内容引用和 `raw=false` 的操作 schema。输出必须引用读取结果中的 `target_id` 或 `target_ids`。长文本和确定性数据优先输出 `content_ref`，不复制到 `content_literal`。
...
- SDT 后代段落不分配 ID、不出现在查询结果中，但仍占用全局序号和表格位置；
...
- tab、普通换行、分页符、分栏符和 CR 投影为对应保留标记，替换后复原为原类型 XML 节点；
- 正文中的 `[[DOCX:` 经转义后可无损往返，匹配边界落在保留标记内部时整项拒绝。

### 15.3 多段落操作

- `replace_para` 把单个目标替换为 `items` 展开后的多个段落，并保持 item 与段内顺序；
- `delete_para` 接受多个目标 ID，并按文档逆序删除；
- `target_ids` 为空、重复或包含不存在的 ID 时整批拒绝；
- 多个操作的目标集合无冲突时可以在同一批次执行。
- 所有段落内容读取接口在 `raw=true` 时返回完整 `<w:p>`，且不出现 `[[DOCX:...]]` 投影；
- 原始插入和替换接受含一个或多个 `<w:p>` 的 fragment，保持顶层顺序，允许任意段内 XML，并拒绝非 `<w:p>` 顶层节点；
- 默认模式把单换行写为 `<w:br/>`，把连续两个及以上换行展开为段落边界；该规则对 literal、file、jsonpath 和 command 内容一致；
- 读取原始 XML、修改后通过 `replace_para(raw=true)` 写回可完成无文本投影参与的往返；LLM schema 不暴露原始模式。
...
- 未命中 XML 节点在 XML 规范化后保持语义及结构不变，ZIP 中其他 entry 内容不变。
...
- [整段替换会移除局部 run 样式和复杂内联结构] → 在结果与 change log 中明确列出警告；需要精确保真时使用段内操作或可信 raw 模式。
- [实例内 ID 不能跨进程持久化] → 明确限定操作生命周期，重新打开文档后强制重新查询锚点。
- [raw 模式允许调用方构造复杂 OOXML] → 仅面向可信 Python 调用方，并在写入前校验 fragment 良构且顶层均为 `w:p`。
- [规范化 XML 只能证明结构与语义保真，不能证明字节一致] → 对其他 ZIP entry 做字节不变检查，对主文档采用规范化结构比较。
- [大文档的 TextMap、索引和快照会增加 CPU/内存占用] → 建立一次性索引、避免逐段扫描整棵 DOM，并以基准测试约束退化。

## Migration Plan

1. 建立 package 骨架、公共模型和结构化异常，不接入现有编辑流程。
2. 实现只读解析、锚点 manifest、表格位置与 TextMap，并用真实 DOCX 固定行为。
3. 实现段内和段落操作、内容来源、预校验、冲突检测与事务回滚。
4. 实现备份、源漂移检测、校验和原子保存。
5. 发布公共 API、quickstart 与 Agent Skill，将调用方迁移为“先查询 ID、后提交操作”。
6. 旧编辑路径仅在新引擎验收通过前保留；切换完成后删除，回滚时恢复调用方到旧版本，不复用新实例中的段落 ID。

## Open Questions

无。实现中如发现 OOXML 结构行为与本文假设不符，必须先用真实 DOCX 验证并更新本设计与对应规格，再继续实现。
